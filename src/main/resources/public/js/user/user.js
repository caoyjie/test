layui.use(['table', 'layer'], function () {
    ...
    $(".search_btn").click(function () {
        ...
        tableIns.reload({
            ...
            where: { //设定异步数据接口的额外参数，任意设
                // 通过文本框/下拉框的值，设置传递的参数
                userName: $("[name='userName']").val() // 用户名称，在user.ftl里面拿
                , email: $("[name='email']").val() // 邮箱，在user.ftl里面拿
                , phone: $("#phone").val() // 手机号，在user.ftl里面拿
            }
            , page: {
                curr: 1 // 重新从第 1 页开始
            }
    ...
    function deleteUsers(userData) {
        ...
        //询问用户是否确认删除
        layer.confirm('您确定要删除选择的记录吗？', {icon: 3, title: '用户管理'}, function (index) {
            //关闭确认框
            layer.close(index);
            //传递的参数是数组
            var ids = "ids=";
    ...
    function openAddOrUpdateUserDialog(id) {
        ...
        if (id != null && id != '') {
            ...
            url+="?id="+id;//传递主键，查询数据，在修改用户信息的时候让数据填充到表单中。
        }
        layui.layer.open({
            type: 2,
            title: title,
