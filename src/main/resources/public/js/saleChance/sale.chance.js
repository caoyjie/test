layui.use(['table', 'layer'], function () {
    ...
    function formatState(state) {
        if (state == 0) {
            return "<div style='color:lightgrey'>未分配</div>";
        } else if (state == 1) {
    ...
    function formatDevResult(devResult) {
        if (devResult == 0) {
            return "<div style='color: lightgrey'>未开发</div>";
        } else if (devResult == 1) {
    ...
    function deleteSaleChance(data) {
        ...
        //询问用户是否确认删除
        layer.confirm('您确定要删除选择的记录吗？', {icon: 3, title: '营销机会管理'}, function (index) {
            //关闭确认框
            layer.close(index);
            //传递的参数是数组
            var ids = "ids=";
